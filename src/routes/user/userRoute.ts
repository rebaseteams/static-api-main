...
export default class UsersRoutes {
  ...
  constructor(auth0 : Auth0Interface, usersService : UsersService) {
    ...
    this.router.get('/roles', auth0.checkAuthorization(), async (req, res, next) => {
      try {
    ...
    this.router.get('/count', auth0.checkAuthorization(), async (req, res, next) => {
      try {
    ...
    this.router.get('/:id', auth0.checkAuthorization(), async (req, res, next) => {
      try {
        const { id } = req.params;
    ...
    this.router.delete('/:id', auth0.checkAuthorization(), async (req, res, next) => {
      try {
    ...
    this.router.get('/:skip/:limit', auth0.checkAuthorization(), async (req, res, next) => {
      try {
    ...
    this.router.get('/pending/:skip/:limit', auth0.checkAuthorization(), async (req, res, next) => {
      try {
    ...
    this.router.patch('/approval', auth0.checkAuthorization(), async (req, res, next) => {
      try {
    ...
    this.router.patch('/roles', auth0.checkAuthorization(), async (req, res, next) => {
      try {
