...
describe('Recommendations', () => {
  describe('DELETE /recommendations/:id', () => {
    ...
    it('should return No userId in header', async () => {
      const formId = 'id1';
      const result = await request(server).delete(`/artists/recommendations/${formId}`);
    ...
    it('should return Invalid User', async () => {
      const formId = 'id1';
      const result = await request(server).delete(`/artists/recommendations/${formId}`).set({ userid: '1234899' });
  ...
  describe('GET /artists/recommendations', () => {
    ...
    it('should return No userId in header', async () => {
      const result = await request(server).get('/artists/recommendations/');
      expect(result.status).toEqual(400);
      expect(result.body).toEqual({ error: 'No UserId in headers!' });
    });

    it('should return Invalid User', async () => {
      const result = await request(server).get('/artists/recommendations/').set({ userid: '1234899' });
      expect(result.status).toEqual(401);
      expect(result.body).toEqual({ error: 'Invalid User' });
    });

    it('should return all recommendations', async () => {
      const result = await request(server).get('/artists/recommendations/').set({ userid: '1238989' });
